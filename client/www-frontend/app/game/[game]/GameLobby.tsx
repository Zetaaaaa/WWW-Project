...
export default function MainLobby() {
    ...
    const playingPlayers = gameData.players.filter((p: any) => p.role !== 'DEALER'); // Gracze uczestniczący
    
    // Oblicz minimalną stawkę do wyrównania
    ...
    return (
        <div className="p-4 flex flex-col gap-4">
            <header className="flex justify-between bg-slate-900 text-white p-4 rounded shadow-md">
                <div>
                    <h1 className="text-xl font-bold">Poker Game</h1>
                ...
                <div className="text-right">
                    ...
                    <p>Twoja rola: <strong>{myRole}</strong></p>
                    {!isDealer && <p>Twoje saldo: <span className="text-green-400 font-bold">${me?.money}</span></p>}
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {playingPlayers.map((p: any) => (
                    <div key={p.uuid} className={`p-4 rounded border bg-slate-50 ${p.uuid === gameData.activePlayerUuid && (gameData.phase === 'BETTING_1' || gameData.phase === 'BETTING_2') ? 'border-yellow-400 shadow-[0_0_15px_rgba(250,204,21,0.5)] bg-yellow-50' : 'border-slate-300'}`}>
                        <h3 className="font-bold flex justify-between items-center text-slate-800">
                            ...
                            <span className={p.folded ? 'text-red-500 font-bold' : 'text-green-700 font-bold'}>
                                {p.folded ? 'Fold' : `$${p.money}`}
                            </span>
                        </h3>
            ...
            {/* --- PANEL AKCJI GRACZA (CIEMNY MOTYW) --- */}
            {!isDealer && !me?.folded && (
            ...
            {gameData.phase === 'SHOWDOWN' && (
                <div className="mt-4 bg-green-100 border-green-500 border-2 p-5 rounded-lg shadow-lg text-black">
                    <h2 className="text-2xl font-bold text-green-800 mb-2">Koniec Rundy!</h2>
                    {gameData.winnerData ? (
